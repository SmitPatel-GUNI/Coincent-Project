<?php
include 'config.php';
if (isset($_POST['Recruiter_signUp'])) {
    $name = $_POST['name'];
    $email = $_POST['email'];
    ...
    $result = mysqli_query($connection,"SELECT MAX(Id) FROM `recruiters` ");
    $row = mysqli_fetch_row($result);
    $recruiter_id = "R_".$row[0]+1;
    
    $search_query = "SELECT * FROM `recruiters` WHERE `Email` = '$email' ";
    ...
    if ($row==1) {
        ...
        $insert_query = "INSERT INTO `recruiters`(`Recruiter_Id`,`Name`, `Email`, `Password`, `Number`) VALUES ('$recruiter_id','$name','$email','$password','$number')";
        ...
        <form action="" method="post">
            <div class="form_group">
                ...
                <div class="btn_group">
                    ...
                    <button class="btn" type="submit" name="Recruiter_signUp">Create Account</button>
                </div>
            </div>
        </form>
        </div>
